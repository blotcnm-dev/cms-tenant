...
class AdminBannerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query->where('bl_promotions.promotions_type', '=','banner');
        ...
        // 페이징 적용
        $banner_contents = $query->paginate(10);
        // 페이지네이션 링크에 현재 쿼리스트링 유지
        $banner_contents->appends($request->query());
...
//
//        echo "<pre>";
//        print_r($banner_contents);
//        echo "</pre>";
//        exit;

        return view('admin.banner.list', compact('banner_contents'));
    }
    ...
    public function downloadExcel(Request $request)
    {
        ...
        $query = DB::table('bl_promotions')
        ->leftJoin('bl_members', 'bl_promotions.admin_id', '=', 'bl_members.member_id')
        ->leftJoin('bl_config', 'bl_promotions.position', '=', 'bl_config.config_id')
        ->select(
            'bl_promotions.*',
            'bl_members.user_name',
            'bl_config.code_name as position_txt'
        );
...
//        debug_error($sql);
        ...
        foreach ($bl_promotions as $item) {
            ...
            $sheet->setCellValue('I'.$row, $item->position_txt);
        ...
        $filename = '배너목록_' . date('Ymd_His') . '.xlsx';
    ...
    public function create()
    {
        ...
        $banner_config = DB::table('bl_config')
            ->select(DB::raw('code_name, config_id'))
            ->where('code_group', 'banner')
            ->get();
...
//
//        echo "<pre>";
//        print_r($banner_config);
//        echo "</pre>";
//        exit;


        return view('admin.banner.create', compact('banner_config'));
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'banner_img' => 'required|mimes:jpg,jpeg,png,pdf|max:10240',
            'is_view' => 'required',
            'position' => 'required',
        ], [
            'title.required' => '제목은 필수 입력입니다.',
            'title.max' => '제목은 최대 255자까지 가능합니다.',
            'banner_img.required' => '이미지는 필수 입력입니다.',
            'banner_img.mimes' => '이미지 파일만 가능합니다.',
            'banner_img.max' => '이미지 용량은 최대 10M까지 업로드 가능합니다.',
            'is_view.required' => '노출 방식을 선택해주세요.',
            'position.required' => '배너위치를 선택해주세요.',
        ]);
        if ($validator->fails()) {
            return handle_validation_response($request, $validator, 'banner.create');
        }

        $img_path = $request->file('banner_img')->store('promotions', 'public');
        ...
        return handle_success_response($request,'배너가 성공적으로 등록되었습니다.','banner.index', []);
    }
    ...
    public function show(Request $request , $id)
    {
        ...
        $banner_content = DB::table('bl_promotions')
            ->leftJoin('bl_members', 'bl_promotions.admin_id', '=', 'bl_members.member_id')
            ->leftJoin('bl_config', 'bl_promotions.position', '=', 'bl_config.config_id')
            ->select(
                'bl_promotions.*',
        ...
        if (!$banner_content) {
            return handle_success_response($request,'존재하지 않는 게시글입니다.','banner.index', []);
        }


        $banner_content = (array) $banner_content;
        $banner_content['is_view_txt'] = ($banner_content['is_view'] === 'always') ? '상시 노출' : '기간 노출';
        $banner_content['is_state_txt'] = ($banner_content['is_state'] === 'Y') ? '사용함' : '사용안함';
        $banner_content = (object) $banner_content;
        ...
        return view('admin.banner.show', compact('banner_content', 'banner_config'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            return handle_validation_response($request, $validator, 'banner.show', ['id' => $id]);
        }
        ...
        if ($request->hasFile('banner_img') && $request->file('banner_img')->isValid()) {
            ...
            if ($banner_content && $banner_content->pc_img) {
                // 파일 존재 → 삭제 가능
                public_delete($banner_content->pc_img);
            }
        } else {
            $img_path = $banner_content->pc_img;
        }
        ...
        try {
            ...
            return handle_success_response($request, '게시글이 성공적으로 업데이트되었습니다.', 'banner.show', ['id' => $id]);
