...
class AdminMemberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('user_name')) {
            $query->where('bl_members.user_name_hash', '=' , hash('sha256', $request->input('user_name') ) );
            //$query->where('bl_members.user_name', 'like', '%' . hash('sha256', $request->input('user_name') ). '%');
        }

        // 전화번호 검색
        if ($request->filled('phone')) {
            $query->where('bl_members.phone_hash', '=' , hash('sha256', $request->input('phone') ) );
    ...
    public function create()
    {
        ...
        $basic_fields = $this->configService->generateBasicFields();
        $etc_fields = $this->configService->generateEtcFields();
    ...
    private function passes($attribute, $value)
    {
        ...
        $query = DB::table('bl_member')->where($this->hashColumn, $hash);

        // 수정시 현재 레코드 제외
        if ($this->ignoreId) {
            $query->where('member_id', '!=', $this->ignoreId);
        }

        $exists = $query->exists();

        // 메시지 설정
        $this->message = $exists ? '이미 사용중인 ' .$fieldNames[$attribute] . '입니다.' : null;
 ...
 */
    ...
    public function show(Request $request , $id)
    {
        ...
        $profileConfig = $this->configService->getProfileConfig();
        // 기본입력사항 필드만들기
        $basic_fields = $this->configService->generateBasicFields($memberArray, [], false, true);
        // 추가입력사항 필드만들기
        $etc_member = DB::table('bl_member_etc')->where('member_id', $id)->first();
        $etc_memberArray = (array)$etc_member;
        $etc_fields = $this->configService->generateEtcFields($etc_memberArray);
