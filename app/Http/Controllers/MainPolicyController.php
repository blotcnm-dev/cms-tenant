...
class MainPolicyController extends Controller
{
     ...
     * 프론트 서비스이용약관
     */
    public function front_policy(Request $request, string $version = ''){
        $query = DB::table('bl_policy_contents')
                    ->where('bl_policy_contents.title', '=', '서비스이용약관')
                    ->where('bl_policy_contents.is_state', '=', 'Y')
                    ->select( 'bl_policy_contents.*' )
                    ->orderBy('bl_policy_contents.version','desc' )
                    ->orderBy('bl_policy_contents.created_at','desc' );


        $policy_selects = $query->get();
        // version이 없으면 빈 문자열이나 기본값 사용
        if (empty($version)) {
        }else{
            $query->where('bl_policy_contents.version','=',$version);
        }
        $policy_contents = $query->first();

        return view('web.policy.policy', compact('policy_contents', 'policy_selects'));
    }

    /**
     * 프론트 개인정보 처리방침
     */
    public function front_privacy(Request $request){
        $query = DB::table('bl_policy_contents')
            ->where('bl_policy_contents.title', '=', '개인정보 처리방침')
            ->where('bl_policy_contents.is_state', '=', 'Y')
            ->select( 'bl_policy_contents.*' )
            ->orderBy('bl_policy_contents.version','desc' )
            ->orderBy('bl_policy_contents.created_at','desc' );
        ...
        return view('web.policy.privacy', compact('policy_contents', 'policy_selects'));
