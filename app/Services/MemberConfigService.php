 ...
 */
    ...
    public function generateField(
        ...
        if($use && $use !== '0'){
            ...
            switch ($fieldType) {
                ...
                case 'date':
                    $html .= '<input type="date" class="common_input" id="' . $id . '" name="' . $name . '" placeholder="' . $placeholder . '" value="' . $value . '" ' . $readonlyAttr . $attrs . '>';
                    break;
                ...
                case 'checkbox':
                    ...
                    if (!empty($options)) {
                        ...
                        foreach ($optionList as $index => $option) {
                            $isChecked = in_array( trim($option), $checkedValues) ? ' checked' : '';
                            $html .= '<label class="chk_input">';
                            $html .= '<input type="checkbox" value="'.$option.'" name="' . $name . '[]"' . $isChecked . $readonlyAttr . $attrs . '>';
                            $html .= '<span>' . $option . '</span>';
                            $html .= '</label>';
                        }
                    }
     ...
     * @return array
     */
    public function generateBasicFieldsUseFront($userData = null, array $attributes = [], bool $readonly = false,  bool $excludePassword = false)
    {
        $basicConfig = $this->getBasicConfig();
        $basic_fields = [];
...
//            echo "2-->[".$value."]<br>";
            $readonly = false;
            if($item->code === 'user_id' && $value != ''){
                $readonly = true;
            }

            $basic_fields[$key] = $this->generateFieldUseFront(
                $item->use,
                $item->code,
                $item->code,
                $item->code_name,
     ...
     * @return array
     */
    public function generateEtcFieldsUseFront($userData = null, array $attributes = [], bool $readonly = false)
    {
        $etcConfig = $this->getEtcConfig()->first();
        ...
        if (isset($etcConfig->value) && !empty($etcConfig->value)) {
            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($etcConfigData)) {
                foreach ($etcConfigData as $item) {
                    ...
                    $etc_fields[$fieldId] = $this->generateFieldUseFront(
                            $item->is_active ?? false ? '1' : '0',
                        $fieldId,
                        $fieldName,
                        $item->name_kr,
     ...
     * @return string HTML 마크업
     */
    public function generateFieldUseFront(
        $use,
        string $id,
        string $name,
        string $label,
        ...
        if($use && $use !== '0'){
            ...
            $html .= '<label for="' . $id . '" '. ($required && $required !== '0' ? ' class="required"'  : '') .' >' . $label. '</label>';
            ...
            switch ($fieldType) {
                case 'input':
//                    if($id == 'user_id' && $value != ''){
//                        $html .= '<input type="text" id="' . $id . '" name="' . $name . '" placeholder="' . $placeholder . '" value="' . htmlspecialchars($value) . '" ' . $readonlyAttr . $attrs . '>';
//                    }else{
//                        $html .= '<input type="text" id="' . $id . '" name="' . $name . '" placeholder="' . $placeholder . '" value="' . htmlspecialchars($value) . '" ' . $readonlyAttr . $attrs . '>';
//                    }
                ...
                case 'date':
                    $html .= '<div class="calander_input_box">';
                    $html .= '<input type="date" id="' . $id . '" name="' . $name . '" placeholder="' . $placeholder . '" value="' . $value . '" ' . $readonlyAttr . $attrs . '>';
                    $html .= '</div>';
                    break;

                case 'password':
                    $html .= '<input type="password"  id="' . $id . '" name="' . $name . '" placeholder="' . $placeholder . '"' . $readonlyAttr . $attrs . '>';
                    break;
                ...
                case 'radio':
                    ...
                    if (!empty($options)) {
                        ...
                        foreach ($optionList as $index => $option) {
                            ...
                            $html .= '<input type="radio" id="id_'.$option.'" value="'.$option.'" name="' . $name . '"' . $isChecked . $readonlyAttr . $attrs . '>';
                            $html .= '<label for="id_'.$option.'" class="gender_label">'. $option . '</label>';
                        }
                    }
                    $html .= '</fieldset>';
                    break;
            ...
            $html .= '<p id="'.$id.'-error" class="error_notice"></p>';
            $html .= '</div>';
        }
        return $html;
    }
