...
@section('required-page-title', '배너 목록')
...
@section('required-page-main-content')
    <main>
        <div id="wrap">
            ...
            <div class="container">
                <!-- 조건 검색 S -->
                <form action="{{ route('banner.index') }}" method="GET" id="searchForm">
                ...
                <!-- 결과 조회 S -->
                <div class="search_result  @if($banner_contents->total() < 1 ) no_result @endif">
                    <div class="result_top">
                        <div class="flex">
                            <p class="tit">검색결과</p>
                            <p class="count pc_block">총 <span>{{ $banner_contents->total() }}</span>건의 결과가 조회 되었습니다.</p>
                        </div>
                        @if($banner_contents->total() > 0 )
                            <a href="{{ route('banner.download.excel') }}?{{ http_build_query(request()->query()) }}" class="fill_btn download">
                                <span>엑셀 다운로드</span>
                            </a>
                        @endif
                    </div>
                    <div class="result_list">
                        @if($banner_contents->total() < 1 )
                        <div class="nodata">
                            <div>
                                <p>조회된 데이터가 없습니다.</p>
                            </div>
                        ...
                        <div class="white_wrap">
                            <div class="common_table">
                                ...
                                <ul role="list" class="tbody">
                                    @php
                                        $startNumber = $banner_contents->total() - ($banner_contents->perPage() * ($banner_contents->currentPage() - 1));
                                    @endphp

                                    @foreach($banner_contents as $index => $item)
                                        <li role="listitem" class="{{ ( $item->is_state  == 'Y')  ? '' : 'hidden' }}">
                                            ...
                                            <div class="title left">
                                                <span>제목</span>
                                                <p>
                                                    <a href="{{ route('banner.show', $item->promotions_id) }}">
                                                        {{ ( $item->title )  ?? '' }}
                                                    </a>
                                                </p>
                                            </div>
                                            ...
                                            <div class="management">
                                                <form action="{{ route('banner.destroy', $item->promotions_id) }}" method="POST" onsubmit="return confirm('정말 삭제하시겠습니까?');">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <!-- 페이지네이션 S -->
                            {{ $banner_contents->links('vendor.pagination.default') }}
                            <!-- 페이지네이션 E -->
                        @endif
                        </div>
                        <div class="bottom_btn">
                            <a href="{{ route('banner.create') }}" class="border_btn register">
                                <span>등록</span>
                            </a>
                        </div>
                    </div>
                    @if($banner_contents->total() > 0 )
                        <a href="{{ route('banner.download.excel') }}?{{ http_build_query(request()->query()) }}" class="border_btn blue download">
                            <span>엑셀 다운로드</span>
                        </a>
                    @endif
                </div>
