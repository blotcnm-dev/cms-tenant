...
@section('required-page-title', '자주 묻는 질문 상세')
    ...
    <main>
        <div id="wrap" class="white">
            ...
            <div class="page_title">
                ...
                <h2 class="title">자주 묻는 질문 상세</h2>
            </div>
            <!-- 페이지 타이틀 E -->

            <!-- 컨텐츠 S -->
            <div class="container">
                <form id="mainForm" action="{{ route('faq.update', [$boards->post_id]) }}" method="POST" enctype="multipart/form-data" class="max_width">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="category" value="{{$boards->category}}">
                    ...
                    <div class="input_box gray_box">
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <div class="custom_select_1 js_custom_select">
                                    <input type="text" class="common_input select_value" placeholder="1차 분류" data-value="{{$boards->category}}" name="category_tmp" value="{{$boards->category_tmp}}" readonly>
                                    <ul role="list">
                        ...
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <input type="text" class="common_input" id="q_title" placeholder="제목을 입력하세요" name="subject" value="{{ $boards->subject }}">
                            </div>
                        </div>
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <div class="uploadFile_box">
                                    ...
                                    <div class="type_txt">
                                        ...
                                        <div class="txt_box">
                                            <div class="info_box">
                                                <input type="hidden" name="post_hidden_file[]" value="{{$file->id}}">
                                                <p class="name">{{$file->fname}}</p>
                                                <span class="capacity">{{$file->fsize}}MB</span>
                                                <button type="button" class="del_btn" aria-label="삭제하기">삭제</button>
                                            </div>
                                        </div>
                                        @endforeach
                                    </div>
                        ...
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <!-- 에디터 들어갈 부분 -->
                                <textarea id="contents" name="contents" class="editor">{{ $boards->content }}</textarea>
                           </div>
                            <div id="contents_error" class="error_msg"></div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title" for="q_date">등록일자</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" id="q_date" value="{{ format_date( $boards->created_at ) }}" readonly>
                            </div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title" for="q_hits">조회수</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" id="q_hits" value="{{$boards->hits}}" readonly>
                            </div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title" for="q_writer">작성자</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" id="q_writer" name="writer_name" value="{{$boards->writer_name}}">
                            </div>
                        </div>
                        <div class="input_item half">
                            ...
                            <div class="inner_box flex gap_input">
                                <label class="radio_input">
                                    <input type="radio" name="is_display" id="show_y" value="1" {{ ( $boards->is_display  == 1)  ? 'checked' : '' }}>
                                    <span>노출</span>
                                </label>
                                <label class="radio_input">
                                    <input type="radio" name="is_display" id="show_n" value="0" {{ ( $boards->is_display  == 0)  ? 'checked' : '' }}>
                                    <span>비노출</span>
                                </label>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.addEventListener('click', function(event) {
                // 클릭된 요소가 .del_btn 이 아니면 무시
                if (!event.target.classList.contains('del_btn')) {
                    return;
                }

                // 가장 가까운 상위 <div>를 찾아 제거
            ...
            document.getElementById('mainForm').addEventListener('submit', function(e) {
                ...
                const url = '{{ route('faq.update', [$boards->post_id]) }}';
                ...
                fetch( url , {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            // 성공 시 처리
                            window.location.href = data.redirect || '{{ route('faq.show', [$boards->post_id]) }}';
                        } else {
