...
@section('required-page-title', '1:1 문의 목록')

@section('required-page-header-css')
    <link rel="stylesheet" href="/src/style/support/inquiryList.css">
@stop
...
@section('required-page-main-content')
    <main>
        <div id="wrap">
            ...
            <div class="container">
                <!-- 조건 검색 S -->
                <form action="{{ route('inquiry.index') }}" method="GET" id="searchForm">
                    ...
                    <div class="search_box">
                        <div class="input_box">
                            ...
                            <div class="input_item half">
                                <label class="input_title">처리 상태</label>
                                <div class="inner_box">
                                    <div class="custom_select_1 js_custom_select">
                                        <input type="text" class="common_input select_value" placeholder="전체" readonly data-value="" name="status" value="{{ request('status') ?? '' }}">
                                        <ul role="list">
                                            <li role="listitem" data-value="all">전체</li>
                                            <li role="listitem" data-value="READY">답변대기</li>
                                            <li role="listitem" data-value="COMPLETE">답변완료</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="search_result  @if($boards->total() < 1 ) no_result @endif">
                    ...
                    <div class="result_list">
                        ...
                        @else
                            <form id="listForm" action="{{ route('inquiry.destroy') }}" method="POST">
                                @csrf
                                @method('DELETE')
                            ...
                            <div class="common_table">
                                <div class="thead">
                                    <div class="check">
                                        ...
                                        @if($boards->total() > 0 )
                                            <button type="button" id="deleteBtn" class="border_btn white" disabled>
                                                <span>삭제</span>
                                            </button>
                                        @endif
                                    </div>
                                ...
                                @foreach($boards as $index => $item)
                                    <li role="listitem">
                                        <a href="{{ ( $item->reply_status  == 'READY')  ? route('inquiry.show', [$item->post_id]) : route('inquiry.view', [$item->post_id]) }}">
                                            <div class="check">
                                                <label class="chk_input">
                                            ...
                                            <div class="state">
                                                <span>노출 여부</span>
                                                <p>{{ ( $item->reply_status  == 'READY')  ? '답변대기' : '답변완료' }}</p>
                                            </div>
                                            <div class="date">
                                                <span>등록 일자</span>
                    ...
                    @if($boards->total() > 0 )
{{--                        <a href="{{ route('inquiry.download.excel') }}?{{ http_build_query(request()->query()) }}" class="border_btn blue download">--}}
{{--                            <span>엑셀 다운로드</span>--}}
{{--                        </a>--}}
                    @endif
                </div>
