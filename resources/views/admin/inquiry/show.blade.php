...
@section('required-page-main-content')
    <main>
        <div id="wrap" class="white">
            ...
            <!-- 컨텐츠 S -->
            <div class="container">
                <form id="mainForm" action="{{ route('inquiry.update', [$boards->post_id]) }}" method="POST" enctype="multipart/form-data" class="max_width">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="is_email" value="{{$boards->is_email}}">
                    <input type="hidden" name="email" value="{{safe_decrypt($boards->email)}}">
                    <div class="input_box gray_box">
                        <div class="input_item half">
                            <label class="input_title">분류</label>
                            <div class="inner_box">
                                <input type="text" name="category_tmp" class="common_input" value="{{ $boards->category_tmp }}" readonly>
                            </div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title">등록 일자</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" value="{{ format_date( $boards->created_at ) }}" readonly>
                            </div>
                        </div>
                        <div class="input_item">
                            <label class="input_title">제목</label>
                            <div class="inner_box">
                                <input type="text" name="subject" class="common_input" value="{{ $boards->subject }}" readonly>
                            </div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title">이메일</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" value="{{ safe_decrypt($boards->email) }}" readonly>
                            </div>
                        </div>
                        <div class="input_item half">
                            <label class="input_title">첨부파일</label>
                            <div class="inner_box">
                                <div class="common_input file">
                                    @foreach($post_files as $file)
                                        <a href="{{$file->path}}">{{$file->fname}}</a>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        <div class="input_item">
                            <label class="input_title">문의</label>
                            <div class="inner_box">
                                <div class="common_textarea">
                                    <textarea name="content" style="display: none">{{ $boards->content }}</textarea>
                        ...
                        <div class="input_item select_answer type_editor">
                            <label class="input_title">답변</label>
                            <div class="inner_box">
                                <!-- 에디터 들어갈 부분 -->
                                <textarea id="contents" name="contents" class="editor">{{ $boards->recontent }}</textarea>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('mainForm').addEventListener('submit', function(e) {
                ...
                const url = '{{ route('inquiry.update', [$boards->post_id]) }}';
                ...
                fetch( url , {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            window.location.href = data.redirect || '{{ route('inquiry.index') }}';
                        } else {
