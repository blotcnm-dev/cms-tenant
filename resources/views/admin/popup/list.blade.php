...
@section('required-page-title', '팝업 목록')

@section('required-page-header-css')
    <link rel="stylesheet" href="/src/style/siteManagement/bannerList.css">
@stop
...
@section('required-page-main-content')
    <main>
        <div id="wrap">
            ...
            <div class="container">
                <!-- 조건 검색 S -->
                <form action="{{ route('popup.index') }}" method="GET" id="searchForm">
                    <input type="hidden" name="sort_order" value="{{ request('sort_order', 'edate__desc') }}" >
                    <div class="search_box">
                        <div class="input_box">
                            ...
                            <div class="input_item half date">
                                <label class="input_title">등록 날짜</label>
                                <div class="inner_box">
                                    <div class="calendar_input">
                            ...
                            <div class="input_item half">
                                ...
                                <div class="inner_box">
                                    <div class="custom_select_1 js_custom_select">
                                        <input type="text" class="common_input select_value" placeholder="{{ request('device') ?? '전체' }}" readonly data-value="" name="device">
                                        <ul role="list">
                                            <li role="listitem" data-value="A">전체</li>
                                            <li role="listitem" data-value="P">데스크톱</li>
                                            <li role="listitem" data-value="M">모바일</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                ...
                <!-- 결과 조회 S -->
                <div class="search_result  @if($popup_contents->total() < 1 ) no_result @endif">
                    <div class="result_top">
                        <div class="flex">
                            <p class="tit">검색결과</p>
                            <p class="count pc_block">총 <span>{{ $popup_contents->total() }}</span>건의 결과가 조회 되었습니다.</p>
                        </div>
                        @if($popup_contents->total() > 0 )
                            <a href="{{ route('popup.download.excel') }}?{{ http_build_query(request()->query()) }}" class="fill_btn download">
                                <span>엑셀 다운로드</span>
                            </a>
                        @endif
                    </div>
                    <div class="result_list">
                        @if($popup_contents->total() < 1 )
                            <div class="nodata">
                                <div>
                                    <p>조회된 데이터가 없습니다.</p>
                                </div>
                            ...
                            <div class="white_wrap">
                                <div class="common_table">
                                    <div class="thead">
                                        ...
                                        <div class="time">
                                            <button type="button" class="sorting_btn">노출 기간</button>
                                        </div>
                                    ...
                                    <ul role="list" class="tbody">
                                        @php
                                            $startNumber = $popup_contents->total() - ($popup_contents->perPage() * ($popup_contents->currentPage() - 1));
                                        @endphp

                                        @foreach($popup_contents as $index => $item)
                                            <li role="listitem" class="{{ ( $item->is_state  == 'Y')  ? '' : 'hidden' }}">
                                                ...
                                                <div class="title left">
                                                    <span>제목</span>
                                                    <p>
                                                        <a href="{{ route('popup.show', $item->promotions_id) }}">
                                                            {{ ( $item->title )  ?? '' }}
                                                        </a>
                                                    </p>
                                                </div>
                                                <div class="time">
                                                    <span>노출 기간</span>
                                                    <p>{{ format_date ($item->sdate) }} ~ {{ format_date ($item->edate) }}</p>
                                                </div>
                                                <div class="show">
                                                    <span>노출 여부</span>
                                                ...
                                                <div class="management">
                                                    <form action="{{ route('popup.destroy', $item->promotions_id) }}" method="POST" onsubmit="return confirm('정말 삭제하시겠습니까?');">
                                                        @csrf
                                                        @method('DELETE')
                                ...
                                <!-- 페이지네이션 S -->
                                {{ $popup_contents->links('vendor.pagination.default') }}
                                <!-- 페이지네이션 E -->
                                @endif
                            </div>
                            <div class="bottom_btn">
                                <a href="{{ route('popup.create') }}" class="border_btn register">
                                    <span>등록</span>
                                </a>
                            </div>
                    </div>
                    @if($popup_contents->total() > 0 )
                        <a href="{{ route('popup.download.excel') }}?{{ http_build_query(request()->query()) }}" class="border_btn blue download">
                            <span>엑셀 다운로드</span>
                        </a>
                    @endif
                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            sortButtons.forEach((button, index) => {
                // index 0: 노출기간
                const field = index === 0 ? 'edate' : 'edate';
            ...
            if (mobileSelect) {
                ...
                selectItems.forEach((item, index) => {
                    item.addEventListener('click', function() {
                        ...
                        setSortOrder('edate', newDirection); // 모바일에서는 등록일자 기준으로만 정렬
                    });
                });
            }
        });
