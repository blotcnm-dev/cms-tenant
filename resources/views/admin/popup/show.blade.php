...
@section('required-page-title', '팝업 상세')
    ...
    <main>
        <div id="wrap" class="white">
            ...
            <div class="page_title">
                ...
                <h2 class="title">팝업 상세</h2>
            </div>
            <!-- 페이지 타이틀 E -->

            <!-- 컨텐츠 S -->
            <div class="container">
                <form id="mainForm" action="{{ route('popup.update', $popup_content->promotions_id) }}" method="POST" enctype="multipart/form-data" class="max_width">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="promotions_type" value="popup">
                    <input type="hidden" name="is_view" value="{{$popup_content->is_view}}">
                    <input type="hidden" name="is_state" value="{{$popup_content->is_state}}">
                    <input type="hidden" name="position" value="{{$popup_content->position}}">
                    <div class="input_box gray_box">
                        <div class="input_item">
                            <label class="input_title" for="popup_title">제목</label>
                            <div class="inner_box">
                                <input type="text" class="common_input" id="popup_title" placeholder="제목을 입력하세요" name="title" value="{{ $popup_content->title }}">
                            </div>
                        ...
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <div class="uploadFile_box">
                                    <input type="file" id="popup_img" name="popup_img" accept="image/jpeg, image/png, image/gif" value="{{ $popup_content->pc_img }}">
                                    <div class="type_txt">
                                    ...
                                    <div class="type_img">
                                        <div class="img_box" id="file_box" style="display: {{ ($popup_content->pc_img)  ? 'block' : 'none' }}">
                                            <img src="{{ ($popup_content->pc_img)  ? $popup_content->pc_img : '' }}" alt="" id="preViewImg">
                        ...
                        <div class="input_item url">
                            ...
                            <div class="inner_box flex">
                                <input type="text" class="common_input" id="popup_url" name="path" value="{{ $popup_content->path }}">
                                <label class="chk_input">
                                    <input type="checkbox" name="target" value="_blank" {{ ($popup_content->target)  ? 'checked' : '' }}>
                                    <span>새 창에서 열기</span>
                                </label>
                            </div>
                        </div>
                        <div class="input_item">
                            ...
                            <div class="inner_box">
                                <div class="textarea_count">
                                    <textarea class="common_textarea" id="popup_text" name="info" placeholder="내용을 입력해주세요">{{ $popup_content->info }}</textarea>
                        ...
                        <div class="input_item half">
                            ...
                            <div class="inner_box flex gap_input">
                                <label class="radio_input">
                                    <input type="radio" id="device_1" name="device" value="A" {{ ($popup_content->device == 'A')  ? 'checked' : '' }}>
                                    <span>전체</span>
                                </label>
                                <label class="radio_input">
                                    <input type="radio" id="device_2" name="device"  value="P" {{ ($popup_content->device == 'P')  ? 'checked' : '' }}>
                                    <span>데스크톱</span>
                                </label>
                                <label class="radio_input">
                                    <input type="radio" id="device_3" name="device"  value="M" {{ ($popup_content->device == 'M')  ? 'checked' : '' }}>
                                    <span>모바일</span>
                                </label>
                            </div>
                        </div>
                        <div class="input_item half">
                            ...
                            <div class="inner_box">
                                <div class="custom_select_1 js_custom_select">
                                    <input type="text" id="id_is_view" class="common_input select_value" name="is_view_txt" placeholder="선택" data-value="{{ $popup_content->is_view }}" value="{{ $popup_content->is_view_txt }}" readonly>
                                    <ul role="list">
                        ...
                        <div class="input_item half date w_6">
                            ...
                            <div class="inner_box">
                                <div class="calendar_input">
                                    <input type="date" class="common_input check_is_date" name="sdate" value="{{ $popup_content->sdate }}">
                                </div>
                                <span>~</span>
                                <div class="calendar_input">
                                    <input type="date" class="common_input check_is_date" name="edate" value="{{ $popup_content->edate }}">
                                </div>
                            </div>
                        </div>
                        <div class="input_item half w_4">
                            ...
                            <div class="inner_box">
                                <div class="custom_select_1 js_custom_select">
                                    <input type="text" class="common_input select_value" name="is_state_txt" placeholder="선택" data-value="{{ $popup_content->is_state }}" value="{{ $popup_content->is_state_txt }}" readonly>
                                    <ul role="list">
                        ...
                        <div class="input_item half input_gap">
                            ...
                            <div class="inner_box flex gap_input">
                                <label class="radio_input">
                                    <input type="radio" id="today_1" name="is_today" value="Y" {{ ($popup_content->is_today == 'Y')  ? 'checked' : '' }}>
                                    <span>사용</span>
                                </label>
                                <label class="radio_input">
                                    <input type="radio" id="today_2" name="is_today"  value="N" {{ ($popup_content->is_today == 'N')  ? 'checked' : '' }}>
                                    <span>사용안함</span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="common_bottom_btn fixed">
                        ...
                        <button type="submit" id="submitBtn" class="border_btn modify">
                            <span>수정</span>
                        </button>
                    </div>
                    <!-- 하단 버튼 E -->
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('mainForm').addEventListener('submit', function(e) {
                ...
                const url = '{{ route('popup.update', $popup_content->promotions_id) }}';
