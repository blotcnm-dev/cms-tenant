...
@section('required-page-title', $board_config->board_name.' 수정')
    ...
    <main>
        <section class="container_edit">
            <div class="container w-820">
                <h2 class="section_title">게시판 글수정</h2>
                <form id="mainForm" action="{{ route('boards.update', [$board_config->board_id]) }}" method="POST" enctype="multipart/form-data" class="max_width">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="category" value="{{$boards->category}}">
                    <input type="hidden" name="post_id" value="{{$boards->post_id}}">
                    ...
                    <fieldset class="category_container">
                        ...
                        <select name="category" value="{{ old('category') }}">
                            @foreach($category_sub as $category)
                                <option value="{{ $category->depth_code }}" {{($boards->category === $category->depth_code) ? 'selected':''}}>{{ $category->kname }}</option>
                            @endforeach
                            <!-- 추가 옵션들 -->
                        </select>
                    </fieldset>
                    ...
                    <div class="input_item_container">
                        ...
                        <input type="text" id="title" name="subject" value="{{ $boards->subject }}" placeholder="제목을 입력해주세요." required>
                        @if($board_config->is_secret === 1)
                            <input type="checkbox" name="is_secret" value="{{$boards->is_secret}}" {{($boards->is_secret === 1) ? 'checked':''}}> 비밀글 사용
                            <input type="hidden" name="secret_password" value="{{$boards->secret_password}}">
                        @endif
                    </div>
                        ...
                        <ul role="list" class="type_txt">
                            @foreach($files as $file)
                                <li class="file_item">
                                    ...
                                    <span class="file_name">{{$file->fname}}</span>
                    ...
                    @if($board_config->board_type === 'GALLERY' )
                        <div class="input_item_container file_container">
                            <ul role="list" class="type_img">
                                @foreach($files as $file)
                                    <li class="img_box">
                                        <input type="hidden" name="gallery_hidden_file[]" value="{{$file->id}}">
                                        <img src="{{$file->path}}" alt="미리보기"><br><br>
                                        <button type="button" class="del_btn" aria-label="삭제하기">삭제하기</button><br><br>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <div class="board_controller">
                        ...
                        <button type="submit" id="submitBtn" class="submit_button">수정하기</button>
                    </div>
                </form>
            </div>
        </section>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('mainForm').addEventListener('submit', function(e) {
                ...
                const url = '{{ route('boards.update', [$board_config->board_id]) }}';
